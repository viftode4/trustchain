...
fn wait_ready(h: &mut NodeHandle, timeout: Duration) -> bool {
    ...
    while start.elapsed() < timeout {
        ...
        if c.get(format!("http://127.0.0.1:{}/status", h.http_port))
            .send()
...
fn test_three_node_bilateral_chains() {
    ...
    let pk_a = json_get(a.http_port, "/status")["public_key"]
        .as_str()
        .unwrap()
        .to_string();
    let pk_b = json_get(b.http_port, "/status")["public_key"]
        .as_str()
        .unwrap()
        .to_string();
    let pk_c = json_get(c.http_port, "/status")["public_key"]
        .as_str()
        .unwrap()
        .to_string();
    assert_ne!(pk_a, pk_b);
    ...
    for (port, pk, addr) in [
        (
            a.http_port,
            &pk_b,
            format!("http://127.0.0.1:{}", b.http_port),
        ),
        (
            a.http_port,
            &pk_c,
            format!("http://127.0.0.1:{}", c.http_port),
        ),
        (
            b.http_port,
            &pk_a,
            format!("http://127.0.0.1:{}", a.http_port),
        ),
        (
            b.http_port,
            &pk_c,
        ...
        json_post(
            port,
            "/peers",
            serde_json::json!({"pubkey": pk, "address": addr}),
        );
    }
    ...
    let r1 = json_post(
        a.http_port,
        "/propose",
        serde_json::json!({"counterparty_pubkey": pk_b, "transaction": {"service": "compute"}}),
    );
    ...
    let r2 = json_post(
        b.http_port,
        "/propose",
        serde_json::json!({"counterparty_pubkey": pk_c, "transaction": {"service": "storage"}}),
    );
    ...
    let r3 = json_post(
        a.http_port,
        "/propose",
        serde_json::json!({"counterparty_pubkey": pk_c, "transaction": {"service": "relay"}}),
    );
    eprintln!("A→C completed={}", r3["completed"]);
    std::thread::sleep(Duration::from_millis(500));

    // Verify chains.
    let ca = json_get(a.http_port, &format!("/chain/{pk_a}"))["blocks"]
        .as_array()
        .unwrap()
        .len();
    let cb = json_get(b.http_port, &format!("/chain/{pk_b}"))["blocks"]
        .as_array()
        .unwrap()
        .len();
    let cc = json_get(c.http_port, &format!("/chain/{pk_c}"))["blocks"]
        .as_array()
        .unwrap()
        .len();
    ...
    // Trust scores.
    let tab = json_get(a.http_port, &format!("/trust/{pk_b}"))["trust_score"]
        .as_f64()
        .unwrap();
